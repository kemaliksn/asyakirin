...
<div class="max-w-2xl mx-auto py-10 px-6">
    <div class="bg-white shadow rounded-xl p-6">
        <h1 class="text-2xl font-extrabold text-green-700 mb-4">Invoice Siap</h1>
        <p class="text-gray-600 mb-6">Transaksi berhasil disimpan. Silakan unduh invoice atau kirim via WhatsApp kepada donatur.</p>

        <div class="border rounded-lg p-4 bg-green-50 border-green-200 mb-6">
            <div class="grid grid-cols-1 gap-2 text-sm">
                <div><span class="font-semibold text-green-800">Nomor:</span> {{ $zakat->nomor }}</div>
                <div><span class="font-semibold text-green-800">Nama:</span> {{ $zakat->nama }}</div>
                <div><span class="font-semibold text-green-800">Tanggal:</span> {{ optional($zakat->tanggal)->format('d M Y') }}</div>
                <div><span class="font-semibold text-green-800">Total Uang:</span> Rp {{ number_format($zakat->total_uang, 0, ',', '.') }}</div>
                <div><span class="font-semibold text-green-800">Total Beras:</span> {{ (float) $zakat->total_beras }} Kg</div>
            </div>
        </div>
        ...
        <?php
            ...
            $msg = "Assalamu'alaikum, berikut tautan invoice zakat Anda: $downloadUrl . Terima kasih telah berzakat di ASY-SYAAKIRIIN.";
        ...
        <div class="mt-6 text-sm text-gray-500">
            <a href="/" class="hover:underline">Kembali ke Form</a>
        </div>
    </div>
</div>
</body>
